...
var _ = Describe("RetryOnError", func() {
	...
	It("raises received errors that should not be retried", func() {
		err := RetryOnError(&logger)(nil, "", "", nil, errors.New("boom"), 0)
	...
	It("retries on HTTP 500 error", func() {
		...
		err := RetryOnError(&logger)(nil, "", "", nil, testError, 0)
		Expect(err).To(BeNil())
	...
	It("retries on Network timeouts", func() {
		...
		err := RetryOnError(&logger)(nil, "", "", nil, &netError, 0)
		Expect(err).To(BeNil())
	...
	It("retries on generic network errors", func() {
		...
		err := RetryOnError(&logger)(nil, "", "", nil, &opError, 0)
		Expect(err).To(BeNil())
