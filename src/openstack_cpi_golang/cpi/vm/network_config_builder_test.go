...
func createNetworkConfig(networkService services.NetworkService, bytes []byte, openstackConfig config.OpenstackConfig, cloudProperties properties.CreateVM) (properties.NetworkConfig, error) {
	var networks apiv1.Networks
	err := json.Unmarshal(bytes, &networks)
	Expect(err).ToNot(HaveOccurred())

	networkConfig, err := vm.NewNetworkConfigBuilder(networkService, networks, openstackConfig, cloudProperties).Build()
